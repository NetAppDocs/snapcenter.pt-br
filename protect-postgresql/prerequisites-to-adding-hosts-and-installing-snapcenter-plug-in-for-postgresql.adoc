...
Para executar um comando complementar no plug-in do SnapCenter para PostgreSQL, você deve incluí-lo no `allowed_commands.config` arquivo.

`allowed_commands.config` O arquivo está localizado no subdiretório "etc" do diretório SnapCenter Plug-in para PostgreSQL.
...
Caminho personalizado: `<Custom_Directory>allowed_commands.config`
