...
* Você precisa ter instalado o Java 17 (para SnapCenter 6.2.1 e posteriores) ou
* Java 11 (para SnapCenter 6.2 e anteriores) em seu host Linux ou AIX.
+
** Java da Oracle e OpenJDK são suportados para Linux.
** IBM Java para AIX. Você pode fazer o download em  https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeru runtimes Downloads"^].
+
...
Pacotes de software necessários
 a| 
* Java 17 (Oracle ou OpenJDK) para SnapCenter 6.2.1 e posterior ou
...
* Edite o  `/etc/ssh/sshd_config` arquivo para configurar os algoritmos de código de autenticação de mensagens: MACs hmac-sha2-256 e MACs hmac-sha2-512.
+
...
Pacotes de software necessários
 a| 
* IBM Java 17 para SnapCenter 6.2.1 e posterior ou
* IBM Java 11 para SnapCenter 6.2 e versões anteriores
...
CAUTION: Embora seja permitido criar credenciais após a implantação de hosts e a instalação de plug-ins, a *melhor prática* é criar credenciais depois de adicionar as SVMs e antes de implantar hosts e instalar plug-ins.
...
NOTE: Se você especificar detalhes incorretos ao criar uma credencial, uma mensagem de erro será exibida. Você deve selecionar *Cancelar* e então tentar novamente.
...
. Selecione image:../media/filter_icon.gif["ícone de filtro"], e em seguida selecione o nome do host e o tipo de banco de dados para filtrar os recursos.
+
Em seguida, você pode selecionar image:../media/filter_icon.gif["ícone de filtro"] para fechar o painel de filtros.

. Selecione o banco de dados e, em seguida, selecione *Configurações do banco de dados* > *Configurar banco de dados*.
...
+
Para mais informações, consulte link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["Nós preferenciais na configuração RAC"^].
