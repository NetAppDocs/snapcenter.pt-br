...
+
Pode adicionar parâmetros adicionais clicando em *image:../media/add_policy_from_resourcegroup.gif["adicionar grupo de recursos do formulário de política"]*.
...
== Atualize o IP preferido no host

Após a conclusão da operação de clone, o caminho fornecido pela camada de acesso ao armazenamento (sal) para o clone estará no formato _<nfs_lif_IP>:<JunctionPath>_. Para fornecer o IP preferido, você deve configurá-lo no host usando os comandos SCCLI.

.Passos
. Faça login no host do banco de dados.
...
. Configure o LIF de dados preferido para o SVM.
+
Endereço IP ou FQDN> <SVM Name>
