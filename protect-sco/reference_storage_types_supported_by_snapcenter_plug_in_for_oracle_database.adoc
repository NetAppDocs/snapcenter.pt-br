...
Outros hipervisores
 a| 
O Linux executado em hipervisores KVM, Xen, Hyper-V e hyperscaler é suportado quando o sistema operacional convidado tem acesso TCP/IP direto aos recursos de storage ONTAP, utilizando os seguintes métodos:
...
NOTE: Datastores e apresentações de dispositivos brutos não são suportados fora do VMware.
