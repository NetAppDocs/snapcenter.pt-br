...
Para executar um comando complementar no plug-in SnapCenter para MySQL, você deve incluí-lo no `allowed_commands.config` arquivo.

`allowed_commands.config` O arquivo está localizado no subdiretório "etc" do diretório SnapCenter Plug-in para MySQL.
