...
+
.. Clique image:../media/filter_icon.png["ícone de filtro"] para filtrar a lista de modo a que apenas as operações de cópia de segurança sejam listadas.
...
== Monitore operações de proteção de dados em instâncias do MySQL no painel atividade
...
. Clique image:../media/activity_pane_icon.gif["painel da atividade"]no painel atividade para visualizar as cinco operações mais recentes.
+
