...
.Sobre esta tarefa
* Para ONTAP 9.12.1 e versões anteriores, os clones criados a partir dos snapshots SnapLock Vault como parte da restauração herdarão o tempo de expiração do SnapLock Vault. Os administradores de armazenamento devem limpar manualmente os clones após o tempo de expiração do SnapLock Vault.
...
Por exemplo, se você adicionar HR como uma tag a vários grupos de recursos, poderá encontrar mais tarde todos os grupos de recursos associados à tag HR.
 ...
 a| 
Use formato de nome personalizado para Snapshot
 a| 
...
NOTE: Os pares de chave-valor personalizados são suportados para sistemas de plug-in IBM DB2 Linux e não são suportados para banco de dados IBM DB2 registrado como um plug-in centralizado do Windows.
