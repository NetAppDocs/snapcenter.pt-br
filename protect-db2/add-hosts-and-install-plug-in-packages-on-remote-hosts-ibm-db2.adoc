...
. Clique em *Enviar*.
+
Se você não selecionou a caixa de seleção *Ignorar verificações prévias*, o host é validado para verificar se atende aos requisitos para a instalação do plug-in. O espaço de disco, a RAM, a versão do PowerShell, a versão do .NET, a localização (para plug-ins do Windows) e o Java 17, Java 11 (para SnapCenter 6.2 somente) para plug-ins do Windows e Linux são validados em relação aos requisitos mínimos. Se os requisitos mínimos não forem atendidos, mensagens de erro ou aviso apropriadas serão exibidas.
