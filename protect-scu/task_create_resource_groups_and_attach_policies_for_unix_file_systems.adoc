...
NOTE: Para consistente com falhas do sistema de arquivos, são criados snapshots do grupo de consistência para os LUNs envolvidos no grupo de volumes.
...
NOTE: Se você adicionou diferentes sistemas de arquivos ao grupo de recursos, todos os volumes de diferentes sistemas de arquivos no grupo de recursos são colocados em um grupo de consistência. Para backups consistente com falhas, um snapshot do grupo de consistência é criado por padrão, mesmo que haja apenas um sistema de arquivos ou volume.
